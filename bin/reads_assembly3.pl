...
my $pick_1k_sequence="/datapool/bioinfo/guchaoyang/pipeline/metagenome/bin/sequence.pl";
...
#	print OUT1 "\#\$ \-S /bin/bash\nexport PATH=\"/datapool/software/anaconda3/envs/qiime1/bin/\:\$PATH\"\n";	
	...
	while(<IN>){
		...
		$cmd.="\#\$ \-S /bin/bash\nexport PATH=\"/datapool/software/anaconda3/envs/qiime1/bin/\:\$PATH\"\ncd $dir/athena\n$athena --config $dir/athena/$sample.config  --threads 60 > athena.log\n#rm $sample.unmap.clean.tmp.1.fq\n#rm $sample.unmap.clean.1.fq\n#rm $sample.unmap.clean.tmp.2.fq\n####rm $sample.unmap.clean.2.fq\n";
 		print OUT $cmd."\n";
		my $cmd_1="perl $pick_1k_sequence -contig $outdir/pick_1k_sequence/$sample.fa -length 1000 $dir/athena/results/olc/athena.asm.fa";
		print OUT1 $cmd_1."\n";
	}
...
if ($fqlist && $assemblytype eq "metaspades"){
	...
	open (LIST,">$outdir/assembly.list") || die;
		...
		my $cmd_1="perl $pick_1k_sequence -contig $outdir/pick_1k_sequence/$sample.fa -length 1000 $dir/metaspades/contigs.fasta";
                print OUT1 $cmd_1."\n";
